...
const start = async () => {
      try {
            ...
            const start: any = new Date(lastDisplayAtDateRecord?.displayAt)?.setDate(new Date(lastDisplayAtDateRecord?.displayAt)?.getDate() + 1) || new Date().setDate(new Date().getDate() + 1);
            ...
            for (let i = 0; i < missingDatesReadsOfDays.length; i++) {
                  ...
                  console.log('date item -', dates[i])
                  console.log('item - ', item)
                  // await ReadsOfDayModel.findOneAndUpdate({ _id: item._id }, { displayAt: dates[i] })
            }
            console.log('JOB(✅) Reads of day display date filling executed successfully!');
      } catch (error: any) {
            console.log('JOB(🔴) Reads of day display date filling Error is - ', error.message);
      }
};
