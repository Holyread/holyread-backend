...
(async () => {
      try {
            ...
            const copyFile = async (
                  ...
                  const bucketName: string = awsBucket[config.NODE_ENV].bucketName
                  
                  return await copyS3File({
                        bucketName,
                        oldKey: `${bucketName}/${awsBucket.bookDirectory}/audio/${name}`,
                        newKey
                  });
            }
                  ...
                  try {
                        if (book.videoFile) {
                              try {
                                    ...
                                    book.videoFile = await next(book.videoFile)
                              } catch ({ message }) {
                                    console.log(
            ...
            console.log('Book files name fixed successfully')

      } catch (e: any) {
            console.log('Fix books files name script execution failed - ', e)
      }
      // return true;
})();
