...
import adminBookSummaryService from '../../../services/admin/book/bookSummary.service'
...
const sendSummaryToKindle = async (req: any, res: Response, next: NextFunction) => {
    try {
        ...
        if (data.coverImage && !data.bookReadFile) {
            data.coverImage = awsBucket[NODE_ENV].s3BaseURL + '/' + awsBucket.bookDirectory + '/coverImage/' + data.coverImage
        ...
        await pdf.create(html).toBuffer(async (err, buffer) => {
            ...
            const fileName: any = await uploadImageToAwsS3(base64Doc, data.title.trim().replace(/ /g, '-'), { ...s3Bucket, documentDirectory: s3Bucket.documentDirectory + '/reads' })
            await adminBookSummaryService.updateBookSummary({ bookReadFile: fileName }, data._id)
            const fileLink = awsBucket[NODE_ENV].s3BaseURL + '/' + awsBucket.bookDirectory + '/reads/' + fileName
            const sentEmailRes = await sentEmail(req.user.kindleEmail, 'Convert', 'Sent book to kindle', fileName, fileLink)
            if (!sentEmailRes) {
                return next(Boom.badRequest(bookSummaryControllerResponse.sendBookToKindleEmailFailure))
            }
            res.status(200).send({ message: bookSummaryControllerResponse.sendBookToKindleSuccess })
        });
    } catch (e: any) {
        next(Boom.badData(e.message))
    }
