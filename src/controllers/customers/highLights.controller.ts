...
const getHighLightsByFilter = async (req: Request | any, res: Response, next: NextFunction) => {
    try {
        ...
        if (params.bookId) {
            filter.bookId = new Types.ObjectId(params.bookId)
        }
        ...
        res.status(200).send({
            message: highLightsControllerResponse.fetchHighLightsSuccess,
            data: params.bookId && data.highLightsBooks.length ? data.highLightsBooks[0] : data,
        });
    } catch (e: any) {
        next(Boom.badData(e.message))
    }
